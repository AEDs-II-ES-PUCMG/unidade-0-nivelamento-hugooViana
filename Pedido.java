...
public class Pedido {
	...
	public void mesclarPedido (outroPedido){
		int novosItens = 0;
	for (int i = 0; i < outroPedido.quantProdutos; i++){
		boolean existe = false;
		for (int j = 0; j < quantProdutos; j++)
		if ( this.produtos[j].equals(outroPedido.produtos[i])){
			existe = true;
			break;
		} 
		if (this.quantProdutos + novosItens > MAX_PRODUTOS) {
			throw new IllegalStateException("capacidade maxima alcancada");
		
		}
		for (int i =0; i < outroPedido.quantProdutos; i++) {
			ItemDePedido itemOutro = outroPedido.produtos[i];
			boolean agrupado = false;
			
			for (int j = 0; j< this.quantProdutos; j++) {
				if (this.produtos[j].equals(itemOutro)) {
					this.produtos[j].setQuantidade(this.produtos[j].getQuantidade() + itemOutro.getQuantidade());
					
					if (itemOutro.getPrecoVenda() < this.produtos[j].getPrecoVenda()) {
						this.produtos[j].setPrecoVenda(itemOutro.getPrecoVenda());
					}
					agrupado = true;
					break;
				}
			...
			if (!agrupado) {
				this.produtos[this.quantProdutos] = itemOutro;
				this.quantProdutos++;
			}
		}
	...
	System.out.println("total geral: R$ " + String.format("%.2f", total));
	}
	}
}